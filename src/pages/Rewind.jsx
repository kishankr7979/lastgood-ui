    ...
    return (
        <div className="p-8 max-w-6xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-text-secondary">Find what changed before production broke.</p>
            </div>
            ...
            <div className="bg-gradient-card border border-white/10 rounded-xl p-6 mb-8 shadow-xl">
                <form onSubmit={handleSearch} className="flex flex-wrap items-end gap-4">
                    ...
                    <div className="w-48">
                        <label className="block text-sm font-medium text-text-muted mb-2">Service (Optional)</label>
                        <input
                            type="text"
                            value={service}
                    ...
                    <div className="w-40">
                        <label className="block text-sm font-medium text-text-muted mb-2">Env (Optional)</label>
                        <input
                            type="text"
                            value={environment}
            ...
            <div className="space-y-6">
                {isLoading && (
                    <LoadingState message="Analyzing timeline..." />
                )}
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                            <div className="bg-gradient-card border border-white/5 rounded-xl p-5">
                                <div className="text-text-muted text-sm mb-1 uppercase tracking-wider">Total Events</div>
                                <div className="text-3xl font-bold text-text-primary">{result.summary.total_events}</div>
                            </div>
                            <div className="bg-gradient-card border border-white/5 rounded-xl p-5">
                                <div className="text-text-muted text-sm mb-1 uppercase tracking-wider">Services Affected</div>
                                <div className="text-3xl font-bold text-text-primary">{Object.keys(result.summary.events_by_service || {}).length}</div>
                            </div>
                            <div className="bg-gradient-card border border-white/5 rounded-xl p-5">
                                <div className="text-text-muted text-sm mb-1 uppercase tracking-wider">Most Recent</div>
                                <div className="text-sm text-text-primary truncate">
                                    {result.summary.most_recent_event ? result.summary.most_recent_event.summary : 'None'}
                                </div>
                            </div>
                        </div>
